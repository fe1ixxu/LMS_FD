...
from fairseq.criterions import FairseqCriterion, MoACriterionConfig, register_criterion
...
@dataclass
class MoALabelSmoothedCrossEntropyCriterionConfig(MoACriterionConfig):
    ...
    moa_gate_loss_wt: Optional[float] = field(
        default=0.0,
        metadata={
            "help": "Weight associated with MoA gate loss in the weighted sum of gate loss and cross entropy loss"
        },
    )
    ...
    cmr_gate_loss_p: Optional[float] = field(
        default=1.0,
        metadata={
            "help": "In CMR loss, encourages fraction of tokens p to be routed to MOA layers"
        },
    )
    cmr_gate_loss_wt: Optional[float] = field(
        default=0.0,
        metadata={
            "help": "Weight associated with CMR loss in the weighted sum of CMR, MoA gate loss, and cross entropy loss"
        },
    )
    moa_cmr_xgpu: Optional[bool] = field(
        default=False,
...
@register_criterion(
    "moa_label_smoothed_cross_entropy",
    dataclass=MoALabelSmoothedCrossEntropyCriterionConfig,
)
class MoALabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    moa_logging_keys = [
    ...
    def __init__(
        ...
        self.gate_loss_weight = moa_gate_loss_wt
        self.gate_loss_combine_method = moa_gate_loss_combine_method
        self.ignore_prefix_size = ignore_prefix_size
        self.report_accuracy = report_accuracy
        self.moa_cmr_xgpu = moa_cmr_xgpu
    ...
    def forward(self, model, sample, reduce=True):
        ...
        loss, nll_loss, moa_loss, cmr_loss, moa_metadata = self.compute_loss(
    ...
    def compute_loss(self, model, net_output, sample, sample_size, reduce=True):
        gate_loss = 0.0
        gate_count = 0
        for l_gate_loss in net_output[1]["moa_gate_loss"]:
        ...
        return loss, nll_loss, gate_loss, cmr_gate_loss, self.get_moa_metadata(model)

    def get_moa_metadata(self, model):
        moa_logging_output = {}
        for key in MoALabelSmoothedCrossEntropyCriterion.moa_logging_keys:
            vals = []
            module_names = []
            for name, module in model.named_modules():
                if isinstance(module, MOALayer):
            ...
            if key == "median_prefix_count_expert1":
                ...
                moa_logging_output["median_prefix_count_expert1_encoder"] = sum(
                    encoder_vals
                ) / len(encoder_vals)
                moa_logging_output[
                    "median_prefix_count_expert1_encoder_1st_layer"
                ...
                moa_logging_output["median_prefix_count_expert1_decoder"] = sum(
                    decoder_vals
                ) / len(decoder_vals)
                moa_logging_output[
                    "median_prefix_count_expert1_decoder_1st_layer"
            ...
            moa_logging_output[key] = sum(vals) / len(vals)
        moa_logging_output["batch_count"] = 1
        return moa_logging_output
    ...
    def reduce_metrics(cls, logging_outputs) -> None:
        ...
        metrics.log_scalar(
            "moa_gate_loss", moa_loss_sum / sample_size, sample_size, round=8
        )
        metrics.log_scalar(
        ...
        all_keys = (
            MoALabelSmoothedCrossEntropyCriterion.moa_logging_keys
            + MoALabelSmoothedCrossEntropyCriterion.secondary_moa_logging_keys
        )
        for key in all_keys:
