...
class MOALayer(Base):
    """
    MOALayer module which implements MixtureOfAdapters as described in Gshard_.
    """
    ...
    def __init__(
        ...
        self.expert_group = (
            ...
            else distributed_utils.get_moe_group(args.moa_expert_count)
        )
        self.all2all_group = (
            all2all_group
            if all2all_group is not None
            else distributed_utils.get_all2all_group(args.moa_expert_count)
        )
        ...
        self.use_tutel = getattr(args, "use_tutel_moa", False)
        self.moa_eval_capacity_max_seqlen = getattr(
            args, "moa_eval_capacity_max_seqlen", False
        )
        self.max_positions = max_positions
        self.tok_dropout = tok_dropout
        self.moa_local_drop = moa_local_drop
    ...
    def forward(
        ...
        if self.moa_eval_capacity_max_seqlen:
        ...
        use_all_to_all = True
        if self.moa_local_drop > 0.0 and self.training:
            if dist.get_rank() == 0:
                use_all_to_all = (
                    dispatched_input.new_empty([]).uniform_() > self.moa_local_drop
                )
            else:
        ...
        return combined_output, {"moa_gate_loss": l_aux}
