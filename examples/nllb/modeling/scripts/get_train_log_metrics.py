...
def get_train_log_metrics(
    ...
    if pattern == "valid_main":
        p3 = subprocess.Popen(
            ["jq", "-r", "-c", f'with_entries( select( (.key | match("valid_main:{direction}(_num_updates|_{metric})")) ) )'],
            stdin=p2.stdout,
            stdout=subprocess.PIPE,
        )
    elif pattern == "train_inner":
        p3 = subprocess.Popen(
            ["jq", "-r", "-c", f'with_entries( select( (.key | match("num_updates|{metric}")) ) )'],
            stdin=p2.stdout,
            stdout=subprocess.PIPE,
        )
    else:
        p3 = subprocess.Popen(
            ["jq", "-r", "-c", f'with_entries( select( (.key | match("{pattern}_num_updates|{pattern}_{metric}")) ) )'],
            stdin=p2.stdout,
            stdout=subprocess.PIPE,
        )
    p1.stdout.close()
