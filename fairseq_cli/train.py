...
def extract_num_params(p):
    return getattr(p, "_orig_size", p).numel()
...
def main(cfg: FairseqConfig) -> None:
    ...
    logger.info(
        "num. non-expert model params: {:,} (num. trained: {:,})".format(
            sum(
                extract_num_params(p)
                for p in model.parameters()
                if not is_expert_param(p)
            ),
            sum(
                extract_num_params(p)
                for p in model.parameters()
    ...
    logger.info(
        "num. expert model params: {:,} (num. trained: {:,})".format(
            sum(
                extract_num_params(p) for p in model.parameters() if is_expert_param(p)
            ),
            sum(
                extract_num_params(p)
                for p in model.parameters()
